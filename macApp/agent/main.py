"""InfraPilot macOS agent entry point.
...
The pkg installer's ``postinstall`` script calls ``--write-config``
(reading ``BACKEND_URL``/``ENROLL_TOKEN`` from its own environment, see
installer/scripts/postinstall) then ``--register-task`` for the launchd
lifecycle -- the macOS equivalent of Inno Setup's CLI-args + post-install
hook flow on Windows.
"""
...
# despite building/importing fine unfrozen. Put macApp/ on sys.path so
# `agent` is importable as a top-level package regardless of how this file
# is invoked (frozen binary, `python agent/main.py`, or `python -m agent.main`).
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--write-config", action="store_true",
                        help="Write config.json from BACKEND_URL/ENROLL_TOKEN env "
                             "vars (postinstall hook) and exit.")
    parser.add_argument("--register-task", action="store_true",
                        help="Register the InfraPilotAgent launchd daemon.")
    parser.add_argument("--unregister-task", action="store_true",
                        help="Remove the InfraPilotAgent launchd daemon.")
    ...
    if args.register_task:
        from .installer_core import register_launch_daemon
        register_launch_daemon(exe_path=Path(sys.executable), interval_minutes=args.interval)
        return 0

    if args.unregister_task:
        from .installer_core import unregister_launch_daemon
        unregister_launch_daemon()
        return 0
