...
const GlobalSearch = () => {
  ...
  return (
    <>
        ...
        <div className="sm:hidden fixed inset-0 z-50 bg-white flex flex-col">
          <div className="flex items-center gap-3 px-4 py-3 border-b border-[#EFEFEF]">
            <FontAwesomeIcon
              ...
              className="text-[#535353] flex-shrink-0"
            />
            <input
              ref={mobileInputRef}
              type="text"
              ...
              className="flex-1 outline-none text-[16px] text-[#3C3C3C] bg-transparent"
            />
            <button
              onClick={closeMobile}
              className="text-[#535353] p-1 flex-shrink-0"
          ...
          {open && debounced.trim().length >= 2 && (
            <div className="flex-1 overflow-auto">{renderResultList()}</div>
          )}
        </div>
      )}
      ...
      <button
        className="sm:hidden w-[34px] h-[34px] flex items-center justify-center text-[#535353]"
      ...
      <div ref={containerRef} className="hidden sm:block relative">
        ...
        />
        {open && debounced.trim().length >= 2 && (
          <div className="absolute right-0 top-[40px] z-50 w-[400px] max-h-[480px] overflow-auto rounded-[10px] bg-white shadow-xl border border-[#EFEFEF]">
            {renderResultList()}
          </div>
        )}
      </div>
