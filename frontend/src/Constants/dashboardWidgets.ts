export type DashboardWidgetConfig = {
  id: string;
  label: string;
  component: string;
  w: number;
  h: number;
  minW?: number;
  minH?: number;
  maxW?: number;
  maxH?: number;
};

export const DASHBOARD_WIDGETS: DashboardWidgetConfig[] = [
  // --- KPI cards ---
  {
    id: "733b7571-b65d-4e63-bbf3-d2deea569b0b",
    label: "widget.activeUsers",
    component: "ActiveUsers",
    w: 3,
    h: 2,
    minW: 2,
    minH: 2,
    maxW: 5,
    maxH: 3,
  },
  {
    id: "e6c96c89-75be-4bc8-8f3c-1db36214a4ad",
    label: "widget.activeDevices",
    component: "ActiveDevices",
    w: 3,
    h: 2,
    minW: 2,
    minH: 2,
    maxW: 5,
    maxH: 3,
  },
  {
    id: "a1b2c3d4-1111-4000-8000-000000000001",
    label: "widget.totalUsers",
    component: "TotalUsers",
    w: 3,
    h: 2,
    minW: 2,
    minH: 2,
    maxW: 5,
    maxH: 3,
  },
  {
    id: "a1b2c3d4-1111-4000-8000-000000000002",
    label: "widget.totalDevices",
    component: "TotalDevices",
    w: 3,
    h: 2,
    minW: 2,
    minH: 2,
    maxW: 5,
    maxH: 3,
  },
  {
    id: "a1b2c3d4-1111-4000-8000-000000000003",
    label: "widget.openTickets",
    component: "OpenTickets",
    w: 3,
    h: 2,
    minW: 2,
    minH: 2,
    maxW: 5,
    maxH: 3,
  },
  {
    id: "a1b2c3d4-1111-4000-8000-000000000004",
    label: "widget.onlineDevices",
    component: "OnlineDevices",
    w: 3,
    h: 2,
    minW: 2,
    minH: 2,
    maxW: 5,
    maxH: 3,
  },
  // --- Chart cards ---
  {
    id: "b66b5c95-181b-4412-9ba9-dbd716d93cbb",
    label: "widget.bitlockerCompliance",
    component: "BitlockerCompliance",
    w: 4,
    h: 5,
    minW: 3,
    minH: 4,
    maxW: 6,
    maxH: 7,
  },
  {
    id: "a1b2c3d4-2222-4000-8000-000000000001",
    label: "widget.devicesByOS",
    component: "DevicesByOS",
    w: 4,
    h: 5,
    minW: 3,
    minH: 4,
    maxW: 6,
    maxH: 7,
  },
  {
    id: "a1b2c3d4-2222-4000-8000-000000000002",
    label: "widget.ticketsByPriority",
    component: "TicketsByPriority",
    w: 4,
    h: 5,
    minW: 3,
    minH: 4,
    maxW: 6,
    maxH: 7,
  },
  {
    id: "a1b2c3d4-2222-4000-8000-000000000006",
    label: "widget.ticketsByState",
    component: "TicketsByState",
    w: 5,
    h: 5,
    minW: 3,
    minH: 4,
    maxW: 7,
    maxH: 7,
  },
  {
    id: "a1b2c3d4-2222-4000-8000-000000000007",
    label: "widget.securityCompliance",
    component: "SecurityCompliance",
    w: 4,
    h: 5,
    minW: 3,
    minH: 4,
    maxW: 6,
    maxH: 7,
  },
  {
    id: "a1b2c3d4-2222-4000-8000-000000000003",
    label: "widget.usersByDepartment",
    component: "UsersByDepartment",
    w: 5,
    h: 5,
    minW: 3,
    minH: 4,
    maxW: 8,
    maxH: 8,
  },
  {
    id: "a1b2c3d4-2222-4000-8000-000000000005",
    label: "widget.devicesByManufacturer",
    component: "DevicesByManufacturer",
    w: 5,
    h: 5,
    minW: 3,
    minH: 4,
    maxW: 8,
    maxH: 8,
  },
  {
    id: "a1b2c3d4-2222-4000-8000-000000000008",
    label: "widget.topApplications",
    component: "TopApplications",
    w: 5,
    h: 5,
    minW: 4,
    minH: 4,
    maxW: 8,
    maxH: 8,
  },
  {
    id: "c4e5a0d0-2222-4000-8000-0000000000c1",
    label: "widget.complianceBaseline",
    component: "ComplianceBaseline",
    w: 4,
    h: 5,
    minW: 3,
    minH: 4,
    maxW: 6,
    maxH: 7,
  },
  {
    id: "c4e5a0d0-2222-4000-8000-0000000000c2",
    label: "widget.cveSummary",
    component: "CveSummary",
    w: 4,
    h: 5,
    minW: 3,
    minH: 4,
    maxW: 6,
    maxH: 7,
  },
  {
    id: "c4e5a0d0-2222-4000-8000-0000000000a1",
    label: "widget.agentScoreboard",
    component: "AgentScoreboard",
    w: 4,
    h: 4,
    minW: 3,
    minH: 3,
    maxW: 6,
    maxH: 6,
  },
  {
    id: "c4e5a0d0-2222-4000-8000-0000000000a2",
    label: "widget.agentSlaQuality",
    component: "AgentSlaQuality",
    w: 4,
    h: 5,
    minW: 3,
    minH: 4,
    maxW: 6,
    maxH: 7,
  },
  // --- New KPI cards ---
  {
    id: "d1e2c3d4-3333-4000-8000-000000000001",
    label: "widget.licensesExpiring",
    component: "LicensesExpiring",
    w: 3,
    h: 2,
    minW: 2,
    minH: 2,
    maxW: 5,
    maxH: 3,
  },
  {
    id: "d1e2c3d4-3333-4000-8000-000000000002",
    label: "widget.staleAgents",
    component: "StaleAgents",
    w: 3,
    h: 2,
    minW: 2,
    minH: 2,
    maxW: 5,
    maxH: 3,
  },
  {
    id: "d1e2c3d4-3333-4000-8000-000000000003",
    label: "widget.ipConflicts",
    component: "IpConflicts",
    w: 3,
    h: 2,
    minW: 2,
    minH: 2,
    maxW: 5,
    maxH: 3,
  },
  {
    id: "d1e2c3d4-3333-4000-8000-000000000004",
    label: "widget.networkBackupStatus",
    component: "NetworkBackupStatus",
    w: 3,
    h: 2,
    minW: 2,
    minH: 2,
    maxW: 5,
    maxH: 3,
  },
  // --- New chart cards ---
  {
    id: "d1e2c3d4-3333-4000-8000-000000000005",
    label: "widget.licenseSeatUtilization",
    component: "LicenseSeatUtilization",
    w: 4,
    h: 5,
    minW: 3,
    minH: 4,
    maxW: 8,
    maxH: 8,
  },
  {
    id: "d1e2c3d4-3333-4000-8000-000000000006",
    label: "widget.ipamSubnetUtilization",
    component: "IpamSubnetUtilization",
    w: 4,
    h: 5,
    minW: 3,
    minH: 4,
    maxW: 8,
    maxH: 8,
  },
  {
    id: "d1e2c3d4-3333-4000-8000-000000000007",
    label: "widget.retentionLegalHold",
    component: "RetentionLegalHold",
    w: 4,
    h: 4,
    minW: 3,
    minH: 3,
    maxW: 6,
    maxH: 6,
  },
  {
    id: "d1e2c3d4-3333-4000-8000-000000000008",
    label: "widget.procurementPipeline",
    component: "ProcurementPipeline",
    w: 6,
    h: 4,
    minW: 4,
    minH: 3,
    maxW: 12,
    maxH: 6,
  },
  // --- Large cards ---
  {
    id: "f65f5dd1-f822-470a-99aa-023d070ba5c3",
    label: "widget.lastScan",
    component: "LastScan",
    w: 6,
    h: 5,
    minW: 4,
    minH: 4,
    maxW: 12,
    maxH: 8,
  },
  {
    id: "a1b2c3d4-2222-4000-8000-000000000004",
    label: "widget.ticketsOverTime",
    component: "TicketsOverTime",
    w: 6,
    h: 5,
    minW: 4,
    minH: 4,
    maxW: 12,
    maxH: 8,
  },
  // --- From the retired Fleet Health page ---
  {
    id: "e2f1a3b4-4444-4000-8000-000000000001",
    label: "widget.newDevices",
    component: "NewDevicesThisWeek",
    w: 3,
    h: 2,
    minW: 2,
    minH: 2,
    maxW: 5,
    maxH: 3,
  },
  {
    id: "e2f1a3b4-4444-4000-8000-000000000002",
    label: "widget.deviceLifecycle",
    component: "DeviceLifecycle",
    w: 4,
    h: 4,
    minW: 3,
    minH: 3,
    maxW: 8,
    maxH: 6,
  },
  {
    id: "e2f1a3b4-4444-4000-8000-000000000003",
    label: "widget.staleAgentsList",
    component: "StaleAgentsList",
    w: 4,
    h: 5,
    minW: 3,
    minH: 4,
    maxW: 6,
    maxH: 8,
  },
];
