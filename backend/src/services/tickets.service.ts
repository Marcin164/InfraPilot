...
export class TicketsService {
  ...
  async createTicket(dto: CreateTicketDto) {
    ...
    // Run any workflow attached to this category. Best-effort — never block
    // ticket creation on a workflow misconfiguration.
    try {
      await this.workflows.runOnCreate(saved);
    } catch (err) {
      this.logger.warn(
  ...
  async createComment(ticketId: any, authorId: any, dto: any) {
    ...
    try {
      ...
      if (
        ...
        await this.dispatcher.dispatch({
          recipientIds: [ticket.requester.id],
